...
class TransformerTrainer:
    ...
    def train (
        ...
        nopeak_mask = torch.cat([nopeak_mask]*(self.model.config.nhead*batch_size), 0)
        ...
        for epoch in range(500):#self.model.config.first_train_epoc):
            ...
            for sts, tgt, inst_label, ks_label in data:
                ...
                for loss_term in range(batch_size):
                    loss += self.get_loss(instances[loss_term,:-1], inst_label[loss_term])
                    loss += self.get_loss(knapsacks[loss_term,:-1], ks_label[loss_term])
                loss /= 2*batch_size
                loss.backward()
                self.optimizer.step()
    ...
    def data_process(
        ...
        for statePrepare in self.statePrepares:
            ...
            for i in range(len(test_acts)-3):
                ...
                ks_labels = torch.tensor([-1]*10, dtype=torch.int)
                inst_labels = torch.tensor([-1]*10, dtype=torch.int)
